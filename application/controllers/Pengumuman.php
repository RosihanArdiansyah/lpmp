...
class Pengumuman extends CI_Controller {
	...
	function index(){
        ...
        if(empty($this->uri->segment(3))){
			$next_page=2;
			$data['canonical']=site_url('pengumuman');
			$data['url_prev']="";
		}elseif($this->uri->segment(3)=='1'){
			$next_page=2;
			$data['canonical']=site_url('pengumuman');
			$data['url_prev']=site_url('pengumuman');
		}elseif($this->uri->segment(3)=='2'){
			$next_page=$this->uri->segment(3)+1;
			$prev_page=$this->uri->segment(3)-1;
			$data['canonical']=site_url('pengumuman/page/'.$this->uri->segment(3));
			$data['url_prev']=site_url('pengumuman/page/'.$prev_page);
		}else{
			$next_page=$this->uri->segment(3)+1;
			$prev_page=$this->uri->segment(3)-1;
			$data['canonical']=site_url('pengumuman/page/'.$this->uri->segment(3));
		...
		$data['site_title'] = "Daftar Pengumuman LPMP Sulawesi Selatan";
		...
		$data['description'] = "Daftar Pengumuman LPMP Sulawesi Selatan";
		$data['link_cannonical'] = site_url('pengumuman');
		$data['title'] = "Daftar Pengumuman LPMP Sulawesi Selatan";
        ...
        $data['main_view'] = 'pengumuman_view';
	...
	function detail($slug){
		...
		if($query->num_rows() > 0){
			...
			$data['post_id']=$kode;
			$data['link_download'] = $q['post_file_uploads'];
			...
			$data['url'] = site_url('pengumuman/'.$q['pengumuman_slug']);
			...
			$v['logo'] =  $site_info->site_logo_header;
			$data['header'] = $this->load->view('header',$v,TRUE);
            $data['main_view'] = 'pengumuman_detail_view';
            // $this->load->view('blog_detail_view',$x);
            $this->load->view('template_main',$data);
		}else{
