...
class Buletin extends CI_Controller {
	...
	function index(){
	    ...
	    $config['base_url'] = base_url() . 'buletin/page/';
        ...
        if(empty($this->uri->segment(3))){
			$next_page=2;
			$data['canonical']=site_url('buletin');
			$data['url_prev']="";
		}elseif($this->uri->segment(3)=='1'){
			$next_page=2;
			$data['canonical']=site_url('buletin');
			$data['url_prev']=site_url('buletin');
		}elseif($this->uri->segment(3)=='2'){
			$next_page=$this->uri->segment(3)+1;
			$prev_page=$this->uri->segment(3)-1;
			$data['canonical']=site_url('buletin/page/'.$this->uri->segment(3));
			$data['url_prev']=site_url('buletin/page/'.$prev_page);
		}else{
			$next_page=$this->uri->segment(3)+1;
			$prev_page=$this->uri->segment(3)-1;
			$data['canonical']=site_url('buletin/page/'.$this->uri->segment(3));
		...
		$data['description'] = "Daftar Buletin / Jurnal LPMP Sulawesi Selatan";
		$data['link_cannonical'] = site_url('buletin');
		$data['site_title'] = "Daftar Buletin LPMP Sulawesi Selatan";
		$data['url'] = site_url('buletin');
		$data['image'] = $site_info->site_logo_big;

        $data['main_view'] = 'buletin_view';
	...
	function detail($slug){
		...
		if($query->num_rows() > 0){
			...
			$data['icon'] = $site_info->site_favicon;
			$data['link_cannonical'] = site_url('buletin');
			$data['url'] = site_url('buletin/'.$q['buletin_slug']);
            ...
            $data['main_view'] = 'buletin_detail_view';
            // $this->load->view('blog_detail_view',$x);
            $this->load->view('template_main',$data);
		}else{
