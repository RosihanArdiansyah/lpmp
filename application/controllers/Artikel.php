...
class Artikel extends CI_Controller {
	...
	function index(){
        ...
        if(empty($this->uri->segment(3))){
			...
			$data['canonical']=site_url('artikel/page/'.$this->uri->segment(3));
			$data['url_prev']=site_url('artikel/page/'.$prev_page);
		}else{
			$next_page=$this->uri->segment(3)+1;
			$prev_page=$this->uri->segment(3)-1;
			$data['canonical']=site_url('artikel/page/'.$this->uri->segment(3));
		...
		$data['site_title'] = "Daftar Artikel LPMP Sulawesi Selatan";
		...
		$data['description'] = "Daftar Artikel LPMP Sulawesi Selatan";
		$data['link_cannonical'] = site_url('artikel');
		$data['title'] = "Daftar Artikel LPMP Sulawesi Selatan";
	...
	function detail($slug){
		...
		if($query->num_rows() > 0){
			...
			$data['icon'] = $site_info->site_favicon;
			$data['link_cannonical'] = site_url('artikel');
			$data['url'] = site_url('artikel/'.$q['post_slug']);
		    ...
		    $data['header'] = $this->load->view('header',$v,TRUE);
            $data['main_view'] = 'artikel_detail_view';
            // $this->load->view('blog_detail_view',$x);
            $this->load->view('template_main',$data);
		}else{
