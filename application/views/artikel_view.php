<div id="main" class="medium-7 large-7 columns">
    ...
    <?php
        $isi_artikel = htmlentities(strip_tags($row->post_contents)); // membuat paragraf pada isi Artikel dan mengabaikan tag html
        $isi = substr($isi_artikel,0,200); // ambil sebanyak 220 karakter
        $isi = substr($isi_artikel,0,strrpos($isi," "));

       
    ?>
    <div class="post post-classic slideUpRun">
        <a title="<?php echo $row->post_title;?>" href="<?php echo site_url('artikel/'.$row->post_slug);?>"
        ...
        <header class="entry-header">
            ...
            <h3 class="entry-title"><a
                    href="<?php echo site_url('artikel/'.$row->post_slug);?>"><?php echo $row->post_title;?></a>
            </h3>
        </header>
